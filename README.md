...
> Auto-Scaler - Open source tool by AetherCodeHQ

`auto-scaling` `cli` `cloud` `devops` `golang` `open-source`

## What is it?

**Auto Scaler** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Auto-Scaler.git
cd Auto-Scaler
...
```bash
# Basic usage
./Auto-Scaler --path ./target

# With options
./Auto-Scaler --path ./target --format json --output report.json

# Verbose mode
./Auto-Scaler --path ./target --verbose
```
...
### Basic scan
```bash
./Auto-Scaler --path ./my-project
```

### JSON report
```bash
./Auto-Scaler --path ./my-project --format json --output report.json
```
...
- name: Run Auto Scaler
  run: ./Auto-Scaler --path . --format json --output report.json
```
